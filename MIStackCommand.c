...
MICommand *
MIStackListLocals(MIPrintValues vals)
{
	MICommand *	cmd;
	
	cmd = MICommandNew("-stack-list-locals", MIResultRecordDONE);
	MICommandAddOption(cmd, MIIntToCString((int)vals), NULL);
	return cmd;
}

MICommand *
MIStackListVariables(MIPrintValues vals)
{
	MICommand *	cmd;
	
	cmd = MICommandNew("-stack-list-variables", MIResultRecordDONE);
	MICommandAddOption(cmd, MIIntToCString((int)vals), NULL);
	return cmd;
}

MICommand *
MIStackListArguments(MIPrintValues vals, int low, int high)
{
	MICommand *	cmd;
...
MICommand *
MIStackListAllArguments(MIPrintValues vals)
{
	MICommand *	cmd;
