	...
	for (MIListSet(results); (result = (MIResult *)MIListGet(results)) != NULL; ) 
	{
		...
		} else 
		{
			str = "";
                ...
                {
			var->name = strdup(str);
		} else if (strcmp(result->variable, "numchild") == 0) 
		{
		...
		{
		        var->exp = strdup(str);
		} else if (strcmp(result->variable, "thread-id") == 0)
		{
		        var->thread_id = (int)strtol(str, NULL, 10);
		}
	}
...
char *
MIGetVarInfoPathExpression(MICommand *cmd)
{
	MIValue *			value;
	MIResult *			result;
	MIResultRecord *	rr;
	...
	for (MIListSet(rr->results); (result = (MIResult *)MIListGet(rr->results)) != NULL; ) {
		value = result->value;
		if (strcmp(result->variable, "path_expr") == 0) {
			if (value->type == MIValueTypeConst) {
				expr = strdup(value->cstring);
			}
		}
